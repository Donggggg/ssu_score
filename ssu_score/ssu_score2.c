...
void ssu_score(int argc, char *argv[]) // 시작,,
{
	char saved_path[BUFLEN]; // 저장된 경로?..
	int i;

	for(i = 0; i < argc; i++){ // 옵션체크 
		if(!strcmp(argv[i], "-h")){
			print_usage();
			return;
		}
	...
	if(argc >= 3 && strcmp(argv[1], "-c") != 0){ //c옵션 없음 
...
int check_option(int argc, char *argv[]) // 옵션을 체크하는 함수 
{
	...
	while((c = getopt(argc, argv, "e:thpc")) != -1) // 옵션처리
	{
		switch(c){ 
			case 'e': // e옵션
				eOption = true;
